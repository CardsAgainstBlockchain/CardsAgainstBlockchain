...
\newfontfamily\comicsans{DejaVu Sans}  % Alternative to Comic Sans MS
...
\foreach \col in {0,...,2} {
	\foreach \row in {0,...,2} {
		...
		\node [below right] at (\LEFTMARGIN + 0.12in + \col * \CARDWIDTH, \TOPMARGIN + 0.72in + \row * \CARDHEIGHT) {\fontsize{20}{20}\selectfont  Blockchain};
	};
};
\end{tikzpicture}
};
